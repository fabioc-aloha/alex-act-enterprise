...
**Status in v1.0.2**: one skill and one namespaced command, published through `alex-mall`. The public surface is stable under the Alex ACT semantic-versioning contract. Content remains evidence-gated by Steward governance.
...
6. Cut a release when a coherent batch is ready (`git tag vX.Y.Z` → GitHub Release → Mall origin record update)
