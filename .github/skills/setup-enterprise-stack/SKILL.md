...
description: "Emit and (with consent) install the Copilot CLI settings block for the public Microsoft ecosystem — Azure, Fabric (consumption/skills/operations/authoring), Power BI, and Microsoft 365 Agents Toolkit. Use when a heir on any Microsoft-subscribed tenant wants the seven public plugins enabled together, or when auditing / repairing the Microsoft ecosystem enablement in `~/.copilot/settings.json`."
lastReviewed: 2026-07-30
---

# Setup Enterprise Stack

Emit the paste-ready `enabledPlugins` + `extraKnownMarketplaces` block for the seven public Microsoft ecosystem plugins any Microsoft-subscribed tenant can use, and optionally install them after explicit user consent.
...
Three modes. Ask the heir which they want; default to (1).

### 1. Emit only (default, safe)

Print the JSON block above with instructions:

> Paste this block into `~/.copilot/settings.json`. If the file already has `extraKnownMarketplaces` or `enabledPlugins` keys, merge — do not overwrite existing entries. After paste, run `copilot plugin install --all` to install every enabled plugin, or run `copilot plugin install <name>@<marketplace>` for each one individually.

No filesystem write. No CLI invocation. Heir owns the paste + install.

### 2. Consent-gated auto-install

Only after explicit "yes, install them" from the heir:

```powershell
# Merge the block into ~/.copilot/settings.json (preserve existing entries)
# then run:
...
Additive settings merge: if `~/.copilot/settings.json` already has an `enabledPlugins` map with unrelated entries, preserve them. If it already has entries for one of the seven, warn but do not overwrite unless the heir also asked to reset.

### 3. Audit only

Read the current `~/.copilot/settings.json`, compare against the target block, produce a table:

| Plugin | Currently enabled? | Marketplace registered? | Action |
|---|---|---|---|
| `azure@azure-skills` | yes/no | yes/no | `install` / `enable-only` / `nothing` |
| `fabric-consumption@copilot-plugins` | yes/no | (built-in) | `install` / `enable-only` / `nothing` |
| ... | | | |

Do not modify anything. Report only. Heir decides what to do with the audit.

## Safety rules

- **Never** overwrite `~/.copilot/settings.json` without explicit consent from the heir.
...
- **Do** warn if the heir already has `enabledPlugins` entries that conflict with the seven (for example, a pre-existing `fabric-authoring@my-fork` — the target block enables `fabric-authoring@copilot-plugins`, which would produce a plugin-name collision the CLI resolves via last-write-wins).
...
| Overwrite existing `~/.copilot/settings.json` | Merge; preserve unrelated keys |
